/*
! major changes in datetime format
*/
...
export class CRMDashboard extends Component {
  setup() {
    ...
    console.log(session);
    this.user_id = session.storeData["res.partner"][1].main_user_id; // * userId v18 -> main_user_id v19
  ...
  async getCompanyCurrency() {
    const result = await rpc("/get/company_currency", {});
    return result;
  }
  ...
  getDates() {
    ...
    if (days == 365) {
      ...
      this.state.startDate = start.toFormat("yyyy-MM-dd hh:mm:ss");
      this.state.endDate = end.toFormat("yyyy-MM-dd hh:mm:ss");
    } else if (days == 90) {
  ...
  async getLostLeadOpps() {
    ...
    for (let i = 1; i <= 12; i++) {
      ...
      const data = await this.orm.searchCount("crm.lead", [
        ...
        ["active", "=", false],
        ["probability", "=", 0],
        ["date_closed", ">=", startDate.toFormat("yyyy-MM-dd hh:mm:ss")],
        ["date_closed", "<", endDate.toFormat("yyyy-MM-dd hh:mm:ss")],
      ]);
  ...
  async getLeadsByMonth() {
    ...
    for (let i = 1; i <= 12; i++) {
      ...
      const data = await this.orm.searchCount("crm.lead", [
        ["type", "=", "lead"],
        ["user_id", "=", this.user_id],
        ["create_date", ">=", startDate.toFormat("yyyy-MM-dd hh:mm:ss")],
        ["create_date", "<", endDate.toFormat("yyyy-MM-dd hh:mm:ss")],
      ]);
  ...
  async getLeadsByCampaign() {
    ...
    console.log("campaigns", new_dataset);
  }
...
CRMDashboard.template = "crm_dashboard.dashboard";
CRMDashboard.components = { Card, ChartRenderer };

registry.category("actions").add("crm_dashboard.dashboard", CRMDashboard);
